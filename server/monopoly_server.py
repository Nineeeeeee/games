#!/usr/bin/env python3
"""Monopoly multiplayer server with AI bot support - supports 2-6 players"""
...
async def safe_send(ws, data):
    """Send JSON to a WS connection, silently ignoring None or errors (AI players have ws=None)"""
    if ws:
        try:
...
async def schedule_ai_turn(room):
    """Auto-play for the AI player whose turn it is. Called as a background task."""
    player_idx = room['turn_index']
    p = room['players'][player_idx]

    # Guard: only for alive CPU players
    if not p.get('is_cpu') or not p['alive']:
        return
    if room['phase'] == 'ended':
        return

    # "Thinking" delay so it doesn't feel instant
    await asyncio.sleep(random.uniform(1.2, 2.0))

    # Re-check after delay (game might have ended or turn changed)
    if room['turn_index'] != player_idx or room['phase'] == 'ended':
        return

    # --- Roll ---
    await handle_game_command(room, player_idx, 'roll', {})
    ...
    if room['phase'] == 'buying':
        cell_idx = p['pos']
        cell = CELLS[cell_idx]
        # AI strategy: buy if very affordable (cheap) or rich enough
        affordable = p['money'] >= cell[2]
        strategic = p['money'] >= cell[2] * 2 or cell[2] <= 300
        if affordable and strategic:
            await handle_game_command(room, player_idx, 'buy', {})
        else:
            await handle_game_command(room, player_idx, 'skip_buy', {})
    ...
    # --- End turn ---
    if room['phase'] == 'done':
        await handle_game_command(room, player_idx, 'end_turn', {})
        # next_turn will schedule the next AI if applicable
...
async def handle_game_command(room, player_idx, cmd, data):
    ...
    if cmd == 'roll':
        if player_idx != turn:
            await safe_send(p['ws'], {'type': 'error', 'msg': '还没轮到你'})
            return
        ...
        # Check if passed start (wrapped around)
        ...
        if cell[1] != 'property' or room['board'][cell_idx] >= 0:
            return
        if p['money'] < cell[2]:
            await safe_send(p['ws'], {'type': 'error', 'msg': '资金不足'})
            return
...
async def next_turn(room):
    ...
    # Send your_turn only to human players
    if not p.get('is_cpu'):
        await safe_send(p['ws'], {'type': 'your_turn'})

    # If it's an AI player, schedule auto-play
    if p.get('is_cpu'):
        asyncio.create_task(schedule_ai_turn(room))
    ...
    try:
        async for msg in ws:
            ...
            if cmd == 'create':
                ...
                r = ROOMS[room]
                if r['started']:
                    await safe_send(ws, {'type': 'error', 'msg': '游戏已开始'})
                    continue
                if len(r['players']) >= r['max_players']:
                ...
                idx = len(r['players'])
                name = data.get('name', AI_NAMES[idx % len(AI_NAMES)])
                r['players'].append({
                    'ws': None, 'name': name, 'color': COLORS[idx],
                    'money': 1500, 'pos': 0, 'props': [], 'alive': True,
                    'is_cpu': True,
                })
                await broadcast(r, {'type': 'room_update', 'players': room_players(r), 'max_players': r['max_players']})
                ...
                if r['started']:
                    await safe_send(ws, {'type': 'error', 'msg': '游戏已开始'})
                    continue
                # Remove the last CPU player (only CPUs, not real players)
                cpu_indices = [i for i, p in enumerate(r['players']) if p.get('is_cpu')]
                if not cpu_indices:
                    await safe_send(ws, {'type': 'error', 'msg': '没有CPU可以移除'})
                    continue
                # Remove the last CPU player
                idx_to_remove = cpu_indices[-1]
                r['players'].pop(idx_to_remove)
                await broadcast(r, {'type': 'room_update', 'players': room_players(r), 'max_players': r['max_players']})
                ...
                # Trigger first turn - if the first player is CPU, schedule AI
                first = r['players'][0]
                if first.get('is_cpu'):
                    asyncio.create_task(schedule_ai_turn(r))
                else:
                    await first['ws'].send(json.dumps({'type': 'your_turn'}))
        ...
        if room and room in ROOMS:
            ...
            if r['started']:
                ...
                alive_humans = sum(1 for p in r['players'] if p['alive'] and not p.get('is_cpu'))
                alive_cpus = sum(1 for p in r['players'] if p['alive'] and p.get('is_cpu'))
                if alive_humans == 0 and alive_cpus <= 1:
                    r['phase'] = 'ended'
